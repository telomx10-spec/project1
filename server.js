...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/" && req.method === "GET") {
    ...
    const html = `
      ...
      <body>
        <h1>📊 Project1 대시보드 <span class="badge">v1.1.0</span></h1>

        <div class="card">
          <h2>서버 정보</h2>
          <p>호스트: ${req.headers.host}</p>
...
const PORT = 3001;
// nginx 리버스 프록시 뒤에서 동작하므로 외부에 직접 노출하지 않고 localhost에만 바인딩
const HOST = "127.0.0.1";
server.listen(PORT, HOST, () => {
  console.log(`Project1 Server running on http://${HOST}:${PORT}`);
  console.log(`nginx 리버스 프록시를 통해 접속하세요`);
});